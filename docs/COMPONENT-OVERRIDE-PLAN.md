...
| 0.4 | **Override test page** ✅ — shipped as `apps/demo/` (standalone Angular 20 app, bootstrapped via `@angular-devkit/build-angular:application`). Output lands in `site/pages/override-tests/` during `pnpm site:build`. Three-column grid: plain Material baseline / skinned Material (overrides applied) / `.ax-*` custom. Theme + density togglable at runtime. | `apps/demo/` |
