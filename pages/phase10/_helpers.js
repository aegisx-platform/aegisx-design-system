/* AegisX DS · Phase 10 — shared helpers (namespace AX10) */
window.AX10 = (function(){
  ...
  const mount = () => {
    const root = document.getElementById('sections-phase10');
    if (!root) return console.warn('[phase10] #sections-phase10 not found');
    for (const fn of builders) {
      try { root.appendChild(fn()); }
      catch (e) { console.error('[phase10]', fn.name || 'builder', e); }
    }
  };
