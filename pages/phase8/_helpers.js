/* AegisX DS · Phase 8 — shared helpers (namespace AX8) */
window.AX8 = (function(){
  ...
  const mount = () => {
    const root = document.getElementById('sections-phase8');
    if (!root) return console.warn('[phase8] #sections-phase8 not found');
    for (const fn of builders) {
      try { root.appendChild(fn()); }
      catch (e) { console.error('[phase8]', fn.name || 'builder', e); }
    }
  };
