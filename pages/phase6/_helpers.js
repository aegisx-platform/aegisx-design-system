/* AegisX DS · Phase 6 — shared helpers (namespace AX6) */
window.AX6 = (function(){
  ...
  const mount = () => {
    const root = document.getElementById('sections-phase6');
    if (!root) return console.warn('[phase6] #sections-phase6 not found');
    for (const fn of builders) {
      try { root.appendChild(fn()); }
      catch (e) { console.error('[phase6]', fn.name || 'builder', e); }
    }
  };
