/* AegisX DS · Phase 9 — shared helpers (namespace AX9) */
window.AX9 = (function(){
  ...
  const mount = () => {
    const root = document.getElementById('sections-phase9');
    if (!root) return console.warn('[phase9] #sections-phase9 not found');
    for (const fn of builders) {
      try { root.appendChild(fn()); }
      catch (e) { console.error('[phase9]', fn.name || 'builder', e); }
    }
  };
